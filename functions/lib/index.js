...
const email_delivery_1 = require("./email-delivery");
...
const internal_report_data_1 = require("./internal-report-data");
const ActivationReportEmail_1 = require("./emails/ActivationReportEmail");
const automation_health_1 = require("./automation-health");
const InternalNotificationEmail_1 = require("./emails/InternalNotificationEmail");
...
const SignupTrialEndingEmail_1 = require("./emails/SignupTrialEndingEmail");
...
exports.completeOnboarding = functions.https.onCall((0, onboarding_1.createCompleteOnboardingHandler)(db));
_a = (0, monthly_roundup_1.createMonthlyRoundupFunctions)({
    db,
...
}), exports.manageMonthlyRoundup = _a.manageMonthlyRoundup, exports.prepareMonthlyRoundup = _a.prepareMonthlyRoundup, exports.sendApprovedRoundups = _a.sendApprovedRoundups;
// ─── PostHog Analytics ──────────────────────────────────────
let _posthog;
function getPostHog() {
    if (!_posthog) {
...
async function captureServerEvent(uid, event, properties) {
    try {
...
function getResend() {
    if (!_resend) {
        ...
        const receiptFor = (key) => db.collection("emailDeliveries").doc(crypto.createHash("sha256").update(key).digest("hex"));
        const send = (0, email_delivery_1.recordedEmailSender)((0, email_delivery_1.reliableEmailSender)(_resend.emails.send.bind(_resend.emails)), {
...
async function createResendContact(email, firstName, uid) {
    ...
    if (existing?.resendContactId)
        return;
    const { data } = (0, email_delivery_1.requireProviderSuccess)(await getResend().contacts.create({
        email,
        firstName,
        properties: { is_pro: String(isEntitledPro(existing || {})), has_logged_trade: String(!!existing?.firstTradeLoggedAt) },
    }));
    if (!data?.id)
        throw new Error("Email provider returned no contact ID");
...
async function updateResendContact(contactId, updates) {
    if (!contactId)
        return;
    try {
        (0, email_delivery_1.requireProviderSuccess)(await getResend().contacts.update({ id: contactId, ...updates }));
    }
    catch (err) {
...
async function fireResendEvent(event, email, payload) {
    (0, email_delivery_1.requireProviderSuccess)(await getResend().events.send({ event, email, payload }));
}
async function sendWelcomeEmail(email, name, uid) {
...
function paymentMethodLabel(charge) {
    ...
    if (details.card?.last4)
        return `Card ending ${details.card.last4}`;
    if (details.type === "link")
        return "Link";
    if (details.type === "paypal")
        return "PayPal";
    if (details.type)
        return details.type.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
    return null;
}
function buildReceipt(opts) {
    const rows = [];
    if (opts.amount)
        rows.push({ label: "Amount paid", value: opts.amount });
...
async function chargeFromPaymentIntent(piId) {
    if (!piId)
        return null;
...
function planLabelFor(planType) {
...
async function receiptFromCheckoutSession(session, planType) {
    try {
        let charge = null;
        let fallbackReceiptUrl = null;
        if (session.mode === "payment") {
            charge = await chargeFromPaymentIntent(session.payment_intent);
        }
...
async function receiptFromLatestInvoice(sub, planType) {
    try {
...
async function sendProUpgradeEmail(email, name, planType, receipt, deliveryKey) {
    const firstName = name?.split(" ")[0] || "trader";
    const planLabel = planLabelFor(planType);
    const html = await (0, components_1.render)(React.createElement(ProUpgradeEmail_1.ProUpgradeEmail, { firstName, planLabel, receipt }));
    await getResend().emails.send({
        from: FROM_EMAIL,
        to: email,
...
async function sendCancellationEmail(email, name, periodEnd, accessEnded, deliveryKey) {
    const firstName = name?.split(" ")[0] || "trader";
    ...
    const html = await (0, components_1.render)(React.createElement(CancellationEmail_1.CancellationEmail, { firstName, endDate, accessEnded }));
    await getResend().emails.send({
        from: FROM_EMAIL,
        to: email,
...
async function sendCheckoutRecoveryEmail(email, name, trialAvailable, uid, sessionId) {
    const firstName = name?.split(" ")[0] || "trader";
...
exports.onUserCreated = functions.auth.user().onCreate(async (user) => {
    ...
    try {
        ...
        await db.collection("users").doc(user.uid).set({ resendAutomationEnrolled: true, resendEnrollmentStatus: "accepted" }, { merge: true });
    }
    catch (err) {
        ...
        await db.collection("users").doc(user.uid).set({ resendEnrollmentStatus: "failed" }, { merge: true });
...
exports.sendTrialEndingEmails = functions.pubsub
    ...
    try {
        ...
        for (const doc of snapshot.docs) {
            if (sent >= MAX_SENDS)
                break;
            const data = doc.data();
            if (data.trialEndingEmailSentAt || data.emailOptOut)
                continue;
            try {
    ...
    for (const doc of signupTrialSnapshot.docs) {
        if (signupSent >= MAX_SENDS)
            break;
        const data = doc.data();
        if (data.signupTrialEndingSentAt || data.emailOptOut || data.signupThrottled)
            continue;
        ...
        try {
            ...
            const html = await (0, components_1.render)(React.createElement(SignupTrialEndingEmail_1.SignupTrialEndingEmail, { firstName, trialEndDate, unsubscribeUrl: getUnsubscribeUrl(doc.id) }));
            await getResend().emails.send({
                from: FROM_EMAIL,
                to: email,
...
function coveredByResendAutomation(data) {
    if (!data.resendAutomationEnrolled)
        return false;
...
exports.sendDay14UpgradeEmails = functions.pubsub
    ...
    for (const doc of allSnapshot.docs) {
        ...
        if (coveredByResendAutomation(data) || data.emailOptOut || data.signupThrottled || isEntitledPro(data) || data.day14UpgradeSentAt || !data.firstTradeLoggedAt)
            continue;
        const email = data.email;
        if (!email)
            continue;
...
exports.sendDay21BackupEmails = functions.pubsub
    ...
    for (const doc of allSnapshot.docs) {
        if (sent >= MAX_SENDS)
            break;
        const data = doc.data();
        if (coveredByResendAutomation(data) || data.emailOptOut || data.signupThrottled || isEntitledPro(data) || data.day21BackupSentAt || !data.firstTradeLoggedAt)
            continue;
        const email = data.email;
        if (!email)
            continue;
...
exports.sendActivationReport = functions
    ...
    do {
        ...
        for (const user of res.users) {
            ...
            if (created >= Date.parse("2026-06-29T00:00:00Z"))
                users.push({ uid: user.uid, created });
        }
    ...
    const cohorts = (0, internal_report_data_1.activationCohorts)(members, now);
    ...
    let failedEmails;
    try {
        automations = await (0, automation_health_1.automationHealth)(process.env.RESEND_API_KEY || "", now - 7 * 86400000);
        failedEmails = await (0, automation_health_1.failedEmailCount)(process.env.RESEND_API_KEY || "", now - 7 * 86400000);
    }
    catch (err) {
    ...
    const html = await (0, components_1.render)(React.createElement(ActivationReportEmail_1.ActivationReportEmail, {
...
exports.sendTrialOfferBatch = functions.https.onCall(async (_data, context) => {
    assertAdmin(context);
    throw new functions.https.HttpsError("failed-precondition", "Trial offers are retired. Subscriptions charge from day one.");
});
// ─── Send Feedback ─────────────────────────────────────────────
exports.sendFeedback = functions.https.onCall(async (data, context) => {
    ...
    if (!SILENT_TYPES.has(type || "")) {
        ...
        await getResend().emails.send({
            ...
            attachments,
            html: await (0, components_1.render)(React.createElement(InternalNotificationEmail_1.InternalNotificationEmail, {
                title: label,
                sender: userName,
                message: message.trim(),
...
function referralTiersGrantedOf(data) {
...
function computeReferralTierGrant(referrerData, newCount) {
    ...
    try {
        ...
        if (referrerUid && !userData?.referralCounted) {
            ...
            if (referrerEmail) {
                ...
                const grantedAfter = tierUpdate.referralTiersGranted
...
exports.trackActivity = functions.https.onCall(async (data, context) => {
    ...
    const uid = context.auth.uid;
    const eventKey = String(data?.event || "");
    if (!(eventKey in ACTIVITY_EVENTS)) {
        throw new functions.https.HttpsError("invalid-argument", "Unknown activity event.");
    }
    const props = sanitizeActivityProps(data?.props);
    try {
        if (eventKey === "session_seen") {
            const userRef = db.collection("users").doc(uid);
            const today = new Date().toISOString().slice(0, 10);
            const snap = await userRef.get();
            if (snap.data()?.lastActiveDay === today)
                return { ok: true, deduped: true };
            await userRef.set({ lastActiveAt: admin.firestore.FieldValue.serverTimestamp(), lastActiveDay: today }, { merge: true });
...
exports.cleanupReferralIsPro = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be signed in.");
    }
    assertAdmin(context);
...
exports.backfillTrialPro = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be signed in.");
    }
    assertAdmin(context);
...
exports.createCheckoutSession = functions.https.onCall(reported("createCheckoutSession", async (data, context) => {
    ...
    const buildSessionParams = (customerId, promoId) => {
        const params = {
            customer: customerId,
    ...
    const createSession = async (customerId) => {
        if (autoPromoId) {
            try {
                return await getStripe().checkout.sessions.create(buildSessionParams(customerId, autoPromoId));
            }
            catch (err) {
        ...
        switch (event.type) {
            ...
            case "customer.subscription.updated": {
                ...
                if (sub.cancel_at_period_end && previous?.cancel_at_period_end === false) {
                    const userRecord = await admin.auth().getUser(firebaseUid);
                    if (userRecord.email) {
                        await sendCancellationEmail(userRecord.email, userRecord.displayName || undefined, subscriptionPeriodEndIso(sub), false, `renewal-cancelled/${event.id}`);
                    }
                }
                ...
                if (prevStatus === "trialing" && sub.status === "active") {
                    await db.collection("users").doc(firebaseUid).set({ trialConvertedAt: new Date().toISOString(), trialOutcome: "converted" }, { merge: true });
                    try {
                        const userRecord = await admin.auth().getUser(firebaseUid);
                        if (userRecord.email) {
            ...
            case "customer.subscription.deleted": {
                ...
                const trialEndedUnconverted = existingDoc?.trialType === "card" && !existingDoc?.trialConvertedAt &&
...
function buildAnalysisPrompts(trades, cur) {
    ...
    const renderBuckets = (label, m, topN = 8) => {
...
function modelTuning(model, temperature) {
...
function completionBudget(model, visibleTokens) {
...
function getAIClient() {
    ...
    if (gatewayKey) {
        return new openai_1.default({ apiKey: gatewayKey, baseURL: AI_GATEWAY_BASE_URL });
    }
    return new openai_1.default({ apiKey: process.env.OPENAI_API_KEY });
}
function aiModelId(model) {
...
const JSON_OUTPUT_TYPES = new Set(["strategy_tagger", "coaching_tips"]);
// ─── Free-Tier AI Quota ───────────────────────────────────────
...
const FREE_AI_LIMITS = { coaching: 5, utility: 20 };
const FREE_AI_AUTO_DAILY_LIMIT = 8;
function freeQuotaSnapshot(data, monthStr, charged) {
...
async function enforceStripeRateLimit(uid, kind) {
...
function entitlementTier(data) {
    if (!isEntitledPro(data))
        return "free";
    if (data?.isPro && data?.subscription?.status !== "on_trial")
        return "pro";
    return "trial";
}
const AI_UTILITY_FEATURES = new Set(["csv_mapping", "import_insight", "screenshot_import"]);
const AI_AUTO_FEATURES = new Set(["coaching_tips", "journal_prompts", "risk_alert"]);
function aiFeatureClass(feature, payload) {
    if (AI_UTILITY_FEATURES.has(feature))
        return "utility";
    if (AI_AUTO_FEATURES.has(feature))
        return "auto";
    // journal_assist is auto after a save (mode "onSave") and manual via "Ask Coach".
    if (feature === "journal_assist" && payload?.mode === "onSave")
        return "auto";
    return "coaching";
}
async function recordAiCall(opts) {
    ...
    console.log(`[ai_call] uid=${opts.uid} feature=${opts.feature} class=${cls} tier=${opts.tier} model=${opts.model} in=${tokensIn ?? "?"} out=${tokensOut ?? "?"}`);
...
const gateHitLastAt = new Map();
async function recordGateHit(uid, gate, tier, extra) {
    const key = `${uid}:${gate}`;
    const now = Date.now();
...
async function checkAndIncrementFreeAI(uid, feature, payload) {
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        if (cls === "auto") {
            if (auto >= FREE_AI_AUTO_DAILY_LIMIT)
                return { denied: "auto" };
    ...
    if (result.denied) {
        ...
        if (result.denied === "utility") {
            throw new functions.https.HttpsError("permission-denied", `You've used all ${FREE_AI_LIMITS.utility} free AI import reads this month. Upgrade to Pro for unlimited.`);
        }
        throw new functions.https.HttpsError("permission-denied", `You've used all ${FREE_AI_LIMITS.coaching} free AI coaching runs this month. Upgrade to Pro for unlimited AI coaching, analysis, and more.`);
    }
...
async function refundAiUsage(uid, featureType, wasPro, freeClass) {
    try {
        if (wasPro && featureType) {
            ...
            await db.runTransaction(async (tx) => {
                ...
                if (freeClass === "auto") {
                    const auto = data?.autoDay === todayStr ? Number(data?.auto) || 0 : 0;
                    if (auto > 0)
                        tx.set(freeUsageRef, { auto: auto - 1 }, { merge: true });
                    return;
                }
...
exports.analyzeTradesAI = functions.https.onCall(reported("analyzeTradesAI", async (data, context) => {
    ...
    const cur = payloadCurrency(data);
...
function buildJournalReviewPrompt(payload) {
    ...
    const entriesBlock = entries.map((e) => `${e.date}${e.dayPnl !== null ? ` (day P&L: ${cur}${e.dayPnl.toFixed(2)})` : ""} | mood: ${e.mood || "n/a"}${e.emotions ? ` | emotions: ${e.emotions}` : ""}${e.tags ? ` | tags: ${e.tags}` : ""}\n${e.title ? `"${e.title}" — ` : ""}${JSON.stringify(e.text)}`).join("\n\n");
    return {
...
function buildJournalAssistPrompt(payload) {
    ...
    if (payload.mode === "onSave") {
        ...
        const emotions = Array.isArray(payload.emotions)
            ? payload.emotions.filter((e) => typeof e === "string").slice(0, 6).join(", ")
            : "";
        return {
...
function buildTradeReviewPrompt(payload) {
    ...
    if (recentTrades && recentTrades.length > 0) {
        context = "\n\nSurrounding trades for context:\n" + recentTrades.map((t, i) => `${i + 1}. ${t.symbol} ${t.side} P&L: ${cur}${t.pnl?.toFixed(2) || "0.00"}`).join("\n");
    }
    return {
...
const aiNum = (v) => (typeof v === "number" && Number.isFinite(v) ? v : 0);
const aiMoney = (cur, v) => `${cur}${aiNum(v).toFixed(2)}`;
const aiPct = (v) => `${aiNum(v).toFixed(1)}%`;
const aiRatioOrNA = (v) => typeof v === "number" && Number.isFinite(v) ? `${v.toFixed(2)}:1` : "n/a";
const aiSigTag = (g) => g?.significant === true ? "" : ` [small sample, n=${aiNum(g?.count)}, not significant]`;
...
function renderMistakeImpact(cur, impact) {
    if (!impact || typeof impact.taggedTrades !== "number" || impact.taggedTrades <= 0)
        return "";
...
function renderAiGroups(cur, label, arr) {
...
const RETAIL_BENCHMARK_LINE = "Grounding: typical retail day traders run a 40-50% win rate, profit factor 1.0-1.5. A win rate is only meaningful alongside the payoff ratio (avg win / avg loss) — 45% with a 2:1 payoff is profitable, 65% with a 0.5:1 payoff loses money. Compare the trader to these benchmarks, not to perfection.";
...
function buildCoachChatPrompt(payload) {
    ...
    const cur = payloadCurrency(payload);
    const n = aiNum;
    const money = (v) => aiMoney(cur, v);
    const pct = aiPct;
    const ratioOrNA = aiRatioOrNA;
    const renderGroups = (label, arr) => renderAiGroups(cur, label, arr);
...
exports.aiAssist = functions.https.onCall(reported("aiAssist", async (data, context) => {
    ...
    try {
        ...
        const completion = await openai.chat.completions.create({
            ...
            ...(JSON_OUTPUT_TYPES.has(request.type) ? { response_format: { type: "json_object" } } : {}),
        });
...
exports.parseScreenshot = functions.https.onCall(reported("parseScreenshot", async (data, context) => {
    ...
    if (usedToday < 0) {
        ...
        throw new functions.https.HttpsError("resource-exhausted", `You've used your ${LIMIT} free screenshot imports. Upgrade to Pro for ${SCREENSHOT_PRO_DAILY} a day.`);
    }
    if (!aiClientConfigured()) {
        throw new functions.https.HttpsError("internal", "AI API key not configured.");
    }
...
- Return only valid JSON with no extra text.`;
    ...
    const refundUse = async () => {
        try {
            ...
            console.error("parseScreenshot refund failed:", e.message);
        }
    };
    let result;
    try {
...
function sanitizeScreenshotTrades(raw) {
    ...
    for (const row of raw.slice(0, 200)) {
        ...
        const pnl = num(r.pnl);
        if (!symbol || !side || entryPrice === null || exitPrice === null || pnl === null)
            continue;
        const str = (v) => (typeof v === "string" ? v.trim().slice(0, 40) : "");
        out.push({
            symbol,
            side,
            entryPrice,
...
async function resolveSyncDoc(uid, key, snap) {
    if (!snap.exists)
        return null;
    const d = snap.data() || {};
    if (!d.chunked)
    ...
    if (!Number.isInteger(count) || count < 1 || count > sync_chunks_1.SYNC_MAX_CHUNKS) {
    ...
    const chunkSnaps = await db.getAll(...Array.from({ length: count }, (_, i) => col.doc((0, sync_chunks_1.syncChunkDocId)(key, i))));
    const parts = [];
    ...
    return (0, sync_chunks_1.joinSyncChunks)(parts);
}
...
exports.syncData = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const prevChunks = prevSnap.exists && prevSnap.data()?.chunked
            ...
            : 0;
        const parts = (0, sync_chunks_1.splitSyncValue)(value);
        const batch = db.batch();
        if (parts.length === 1) {
            ...
            parts.forEach((part, i) => batch.set(col.doc((0, sync_chunks_1.syncChunkDocId)(key, i)), { part, updatedAt }));
        }
        const keepChunks = parts.length === 1 ? 0 : parts.length;
        for (let i = keepChunks; i < prevChunks; i++) {
            batch.delete(col.doc((0, sync_chunks_1.syncChunkDocId)(key, i)));
        }
        await batch.commit();
...
exports.aiStream = functions.https.onRequest(async (req, res) => {
    ...
    const tier = entitlementTier(userDoc.data());
    let freeUsage = null;
    const requestedFeature = endpoint === "analysis" ? "ai_analysis" : String(reqData?.type || "");
    const requestedPayload = endpoint === "analysis" ? undefined : reqData?.payload;
    ...
    if (endpoint === "analysis") {
        ...
        const trades = request.trades.slice(0, 50);
        const cur = payloadCurrency(reqData);
    ...
    try {
        const stream = await openai.chat.completions.create({
            ...
            ...(featureType && JSON_OUTPUT_TYPES.has(featureType) ? { response_format: { type: "json_object" } } : {}),
        });
        ...
        await recordAiCall({
            ...
            tier,
            usage: streamUsage,
            payload: endpoint === "analysis" ? undefined : reqData.payload,
            extra: { streamed: true },
        });
        const meta = {
            usage: { used: usedToday + 1, limit, remaining: limit - (usedToday + 1) },
...
exports.sendBirthdayLifetimeEmails = functions
