 ...
 * product news. Goes to everyone EXCEPT lifetime owners (skipped, not
 * varianted). One-off campaign; dedup field: birthdayUpdateSentAt.
 *
 * Run only AFTER the frontend + functions are deployed with the birthday window
 * and the FTJBIRTHDAY promotion code exists in Stripe live.
 *
 * Usage (run from functions/ directory):
 *   TEST_TO=you@example.com npx ts-node --compiler-options '{"module":"commonjs","esModuleInterop":true,"jsx":"react-jsx"}' scripts/send-birthday-update.ts
 *   DRY_RUN=true  npx ts-node --compiler-options '{"module":"commonjs","esModuleInterop":true,"jsx":"react-jsx"}' scripts/send-birthday-update.ts
...
import { BirthdayLifetimeEmail } from '../src/emails/BirthdayLifetimeEmail'
...
async function sendTest() {
  const html = await render(
    React.createElement(BirthdayLifetimeEmail, {
      firstName: 'Richy',
...
async function main() {
  ...
  // Pro/trialing users are IN (lifetime replaces their subscription); lifetime owners are OUT.
  ...
  let skippedOptOut = 0, skippedThrottled = 0, skippedSent = 0, skippedLifetime = 0
  ...
  for (const user of allUsers) {
    ...
    if (d?.subscription?.planType === 'lifetime') { skippedLifetime++; continue }
  ...
  console.log(`Skipped: ${skippedOptOut} opted out, ${skippedThrottled} throttled, ${skippedSent} already sent, ${skippedLifetime} lifetime owners, ${rejected.length} bad addresses`)
  console.log(`Send cap this run: ${MAX_SENDS === Number.MAX_SAFE_INTEGER ? 'uncapped' : MAX_SENDS}`)
  console.log(`\nTargeting ${candidates.length} users (everyone except lifetime owners)`)
