 ...
 * EVERYONE incl. Pro and lifetime. One-off campaign; dedup field:
 * tradeIdeasAnnouncementSentAt. Planned for Wed 26 Aug 2026, two days before
 * the birthday lifetime send (send-birthday-update.ts).
 *
 * Usage (run from functions/ directory):
 *   TEST_TO=you@example.com npx ts-node --compiler-options '{"module":"commonjs","esModuleInterop":true,"jsx":"react-jsx"}' scripts/send-trade-ideas-announcement.ts
 *   DRY_RUN=true  npx ts-node --compiler-options '{"module":"commonjs","esModuleInterop":true,"jsx":"react-jsx"}' scripts/send-trade-ideas-announcement.ts
...
import { TradeIdeasAnnouncementEmail } from '../src/emails/TradeIdeasAnnouncementEmail'
...
const SUBJECT = 'New in FreeTradeJournal: Trade Ideas, a shared feed of setups with real results'
const DEDUP_FIELD = 'tradeIdeasAnnouncementSentAt'
...
async function sendTest() {
  const html = await render(
    React.createElement(TradeIdeasAnnouncementEmail, {
      firstName: 'Richy',
