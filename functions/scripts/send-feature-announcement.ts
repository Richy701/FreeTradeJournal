/**
 * Feature announcement — position size calculator + market sessions widget
 * (both shipped 11 Aug 2026). Goes to EVERYONE incl. Pro/lifetime — it's
 * product news, not a sales pitch, and the email says "free on every plan".
 * One-off campaign; dedup field: featureAnnouncementSentAt.
 *
 * Usage (run from functions/ directory):
 *   TEST_TO=you@example.com npx ts-node --compiler-options '{"module":"commonjs","esModuleInterop":true,"jsx":"react-jsx"}' scripts/send-feature-announcement.ts
 *   DRY_RUN=true  npx ts-node --compiler-options '{"module":"commonjs","esModuleInterop":true,"jsx":"react-jsx"}' scripts/send-feature-announcement.ts
...
import { FeatureAnnouncementEmail } from '../src/emails/FeatureAnnouncementEmail'
...
const SUBJECT = 'Two new tools in your journal'
const DEDUP_FIELD = 'featureAnnouncementSentAt'
...
async function sendTest() {
  const html = await render(
    React.createElement(FeatureAnnouncementEmail, {
      firstName: 'Richy',
