/**
 * Feature announcement — combined all-accounts view + calendar percent mode
 * (both shipped 14 Aug 2026, v2.78.0). Goes to EVERYONE incl. Pro/lifetime — it's
 * product news, not a sales pitch, and the email says "free on every plan".
 * One-off campaign; dedup field: allAccountsAnnouncementSentAt.
 *
 * Usage (run from functions/ directory):
 *   TEST_TO=you@example.com npx ts-node --compiler-options '{"module":"commonjs","esModuleInterop":true,"jsx":"react-jsx"}' scripts/send-all-accounts-announcement.ts
 *   DRY_RUN=true  npx ts-node --compiler-options '{"module":"commonjs","esModuleInterop":true,"jsx":"react-jsx"}' scripts/send-all-accounts-announcement.ts
...
import { AllAccountsAnnouncementEmail } from '../src/emails/AllAccountsAnnouncementEmail'
...
const SUBJECT = 'All your accounts, one view'
const DEDUP_FIELD = 'allAccountsAnnouncementSentAt'
...
async function sendTest() {
  const html = await render(
    React.createElement(AllAccountsAnnouncementEmail, {
      firstName: 'Richy',
