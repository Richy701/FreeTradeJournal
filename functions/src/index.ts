...
function trialTombstoneRef(email: string) {
...
async function restoreFreeAllowances(uid: string, tombstone: FirebaseFirestore.DocumentData): Promise<void> {
  ...
  await db.runTransaction(async (tx) => {
    ...
    const mergedAi = mergeFreeAiUsage(ai.data(), tombstone.freeAiUsage, monthStr, todayStr);
    if (mergedAi) tx.set(aiRef, mergedAi);
...
export const sendWeeklyDigestEmails = functions
  ...
  .onRun(async () => {
    ...
    const recipients = allSnapshot.docs.filter((doc) => {
      ...
      if (data.emailOptOut || data.weeklyDigestLastSentWeek === currentWeek) return false;
    ...
    const weeklyStats = async (uid: string) => {
    ...
    try {
      switch (event.type) {
        ...
        case "checkout.session.expired": {
          ...
          if (lastSent && Date.now() - lastSent.getTime() < 30 * 24 * 60 * 60 * 1000) break;
...
function buildPositionCheckPrompt(payload: Record<string, any>) {
  const {
    mode, instrument, accountCurrency, balance, riskAmount, riskPercent,
    stopDescription, positionDescription, rewardRisk, stats,
  } = payload;
...
const SYNC_KEYS = ['trades', 'journalEntries', 'goals', 'tradingGoals', 'accounts', 'riskRules', 'onboardingCompleted', 'onboarding', 'propFirmAccounts', 'propFirmTransactions', 'settings', 'coachingFocus'] as const;
...
export const syncData = functions.https.onCall(async (data, context) => {
  ...
  const { key, allowEmpty } = data as { key: string; value: string; allowEmpty?: boolean };
  // Reassigned by the accounts guard below when it rescues still-referenced accounts.
  let value = (data as { value: string }).value;
    ...
    try {
      ...
      if (Array.isArray(incoming)) {
        ...
        for (const raw of [tradesValue, entriesValue]) {
          ...
          try {
            ...
            if (Array.isArray(records)) {
              for (const r of records) if (r?.accountId) referenced.add(r.accountId);
            }
        ...
        if (Array.isArray(prevAccounts)) {
          const rescued = prevAccounts.filter(
            (a: any) => a?.id && !incomingIds.has(a.id) && referenced.has(a.id),
          );
          if (rescued.length > 0) {
            value = JSON.stringify([...incoming, ...rescued]);
            console.warn(
              `[syncData] Rescued ${rescued.length} still-referenced account(s) dropped by an accounts write for ${uid}: ${rescued.map((a: any) => a.id).join(', ')}`,
            );
          }
        }
      }
...
export const deleteUserAccount = functions.https.onCall(async (_data, context) => {
  ...
  let freeAiUsage: FirebaseFirestore.DocumentData | undefined;
  ...
  try {
    if (email !== "unknown") {
      ...
      await db.runTransaction(async (tx) => {
        ...
        const mergedAi = mergeFreeAiUsage(prev?.freeAiUsage, freeAiUsage, monthStr, todayStr);
        tx.set(ref, {
          trialUsed: true,
...
function mtIngestDocId(server: string, login: string, ticket: string): string {
...
export const createMtSyncKey = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
  ...
  await userRef.set(
    { mtSyncKey: key, mtSyncKeyCreatedAt: admin.firestore.FieldValue.serverTimestamp() },
    { merge: true },
  );
  return { key };
});

export const mtSyncPush = functions.https.onRequest(async (req, res) => {
  ...
  const { key, account, deals } = (req.body ?? {}) as {
    key?: string;
    account?: { server?: string; login?: string | number; platform?: string; currency?: string; balance?: number };
  ...
  for (const deal of deals) {
    const ticket = String((deal as any)?.ticket ?? "").trim();
    const symbol = String((deal as any)?.symbol ?? "").trim();
    if (!ticket || !symbol) {
      skipped++;
      continue;
    }
