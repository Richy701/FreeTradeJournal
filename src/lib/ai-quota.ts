...
// Firebase callable errors carry code 'functions/permission-denied'; the
// streaming HTTP path surfaces the server's message text on a 403.
export function isFreeAiQuotaError(err: unknown): boolean {
  ...
  return /free AI queries this month/i.test(message);
}
