...
export function TradingCoach() {
  ...
  const chatAggregates = useMemo(() => computeTradeAggregates(trades as Trade[]), [trades])
      ...
      try {
        ...
        const response = await requestAIAssist({
          ...
          payload: {
            ...
            avgHoldMinutes,
            tradeCount: trades.length,
            // Everything below was already computed client-side and thrown
            // away — the June audit's #1 finding. Same sample-ranked buckets
            // the chat prompt gets, so tips stop being generic.
            avgWin: chatAggregates.avgWin,
            avgLoss: chatAggregates.avgLoss,
            payoffRatio: chatAggregates.payoffRatio,
            avgPlannedRR: chatAggregates.avgPlannedRR,
            rrSampleCount: chatAggregates.rrSampleCount,
            hasEnoughData: chatAggregates.hasEnoughData,
            significanceThreshold: chatAggregates.significanceThreshold,
            strategiesTagged: chatAggregates.strategiesTagged,
            ...
            perSession: chatAggregates.perSession,
            perEmotion: chatAggregates.perEmotion,
            tilt: { score: tiltScore.score, label: tiltScore.label, factors: tiltScore.factors },
            patterns: detectTradingPatterns(trades),
            timeAnalysis: analyzeTimePatterns(trades),
          },
        })
        ...
        // Server now enforces JSON-object mode ({"tips": [...]}); a bare array
