...
// Schedules the FINAL-DAY lifetime email (Friday August 7) — the closing note
// of the retirement sequence. Audience: the Aug 4 final-48h cohort plus anyone
// who signed up after it. The July 21 wave told its 1,358 recipients "I will
// not email you about this again", so anyone carrying foundingMemberEmailId or
// lifetimeRetirementEmailId stays hard-excluded here too. The Aug 4 email made
// no such promise, so its recipients get this one last note.
//
// Timezone-aware: each email is scheduled for 9:00 AM on Friday August 7
...
// America/New_York (the largest market). LATEST caps every slot well before
// the 11:59 PM UTC cutoff so "tonight" is always still true and actionable.
//
...
//
// Usage:
//   node scripts/admin/send-lifetime-lastday.cjs                # dry run (writes nothing)
//   node scripts/admin/send-lifetime-lastday.cjs --live         # schedule for real
//   node scripts/admin/send-lifetime-lastday.cjs --live --limit 5   # smoke test
//
// Idempotent via lifetimeLastDayEmailId — safe to re-run as a top-up, so a
...
const { LifetimeLastDayEmail } = require('../../functions/lib/emails/LifetimeLastDayEmail');
...
const SUBJECT = 'Last day — Lifetime Pro ends tonight';
const CAMPAIGN = 'lifetime_lastday_2026_08';
const PRICING_URL = `https://www.freetradejournal.com/pricing?utm_source=resend&utm_medium=email&utm_campaign=${CAMPAIGN}`;
const SEND_DAY_9AM_UTC = Date.parse('2026-08-07T09:00:00Z');
const EARLIEST = Date.now() + 2 * 60 * 60 * 1000;
const LATEST = Date.parse('2026-08-07T19:00:00Z'); // still ~5h before the 23:59 UTC cutoff
...
async function scheduleOne(uid, email, firstName, scheduledAt) {
  const html = await render(
    React.createElement(LifetimeLastDayEmail, {
      firstName,
...
(async () => {
  ...
  snap.forEach((doc) => {
    ...
    if (d.lifetimeLastDayEmailId) return skipped.alreadyScheduled++;
  ...
  console.log(`Eligible (final48 cohort + post-Aug-4 signups): ${verified.length}\n`);
    ...
    try {
      ...
      try {
        await db.collection('users').doc(u.uid).set(
          { lifetimeLastDayEmailId: emailId, lifetimeLastDayScheduledAt: u.scheduledAt },
          { merge: true }
        );
      } catch (writeErr) {
  ...
  if (failures.length) console.log('Failed users have no lifetimeLastDayEmailId — re-running targets only them.');
