...
try {
  ...
  await page.route('**/*', route => {
    ...
    if (['/dashboard', '/trades', '/journal', '/changelog', '/__review-whats-new'].includes(url.pathname)) return route.fulfill({contentType: 'text/html', body: `<!doctype html><html class="dark"><head><meta name="viewport" content="width=device-width, initial-scale=1"></head><body class="bg-background text-foreground"><div id="root"></div><script type="module">
  ...
  const dialog = page.getByRole('dialog', {name: 'Log a Trade', exact: true});
  const save = dialog.getByRole('button', {name: 'Save trade', exact: true});
  await save.scrollIntoViewIfNeeded();
  assert.equal(await save.locator('svg').count(), 0);
  assert.equal(await save.getAttribute('style'), null);
  assert(await save.isDisabled());
  await save.locator('..').screenshot({path: '/tmp/ftj-dashboard-save-button.png', animations: 'disabled'});
  console.log('PASS: Dashboard save button uses shared styling, has no icon, and still validates required fields.');
