// Start Vite on 5176, then run: node --experimental-strip-types e2e/harness/journal-lesson-check.ts
...
const out = '/tmp/ftj-journal-lesson';
...
try {
  ...
  await page.getByRole('button', {name: 'Quick review', exact: true}).waitFor();
  for (const width of [390, 1280]) {
    await page.setViewportSize({width, height: 1000});
    await page.screenshot({path: `${out}/journal-${width}-dark.png`, animations: 'disabled'});
  }
  await page.getByRole('button', {name: 'Quick review', exact: true}).click();
  ...
  await page.getByLabel('How did you feel?', {exact: false}).selectOption('patient');
  await page.getByLabel('What would you repeat or change?').fill('Wait for confirmation before entering.');
  await page.getByLabel('One lesson for next time', {exact: false}).fill('Wait for confirmation before entering.');
  await page.getByRole('checkbox', {name: 'Carry this lesson into my next plan'}).check();
  for (const width of [390, 1280]) {
    await page.setViewportSize({width, height: 1000});
  ...
  await page.reload();
  await page.getByRole('button', {name: 'Pre-Trade', exact: true}).first().click();
  const reminder = page.getByRole('complementary', {name: 'Lesson for your next trade'});
  await reminder.getByText('Wait for confirmation before entering.', {exact: true}).waitFor();
  for (const width of [390, 1280]) {
    await page.setViewportSize({width, height: 1000});
    await reminder.scrollIntoViewIfNeeded();
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
    await page.screenshot({path: `${out}/reminder-${width}-dark.png`, animations: 'disabled'});
  }
  await page.evaluate(() => document.documentElement.classList.remove('dark'));
  await page.screenshot({path: `${out}/reminder-desktop-light.png`, animations: 'disabled'});
  await page.getByRole('button', {name: 'Close', exact: true}).click();
  await page.getByRole('button', {name: 'Quick review', exact: true}).click();
  assert.equal(await page.getByLabel('One lesson for next time', {exact: false}).inputValue(), 'Wait for confirmation before entering.');
  await page.getByRole('checkbox', {name: 'Carry this lesson into my next plan'}).uncheck();
  ...
  assert.equal(records.filter(record => record.quickReview).length, 1);
  assert.equal(records[0].lessonPinned, false);
  assert.deepEqual(records[0].emotions, ['patient']);
  await page.getByRole('button', {name: 'Close', exact: true}).click();
  await page.getByRole('button', {name: 'Quick review', exact: true}).click();
  await page.getByLabel('One lesson for next time', {exact: false}).fill('An unsaved lesson');
  await page.evaluate(() => {
  ...
  assert.equal(await page.getByLabel('One lesson for next time', {exact: false}).inputValue(), 'An unsaved lesson');
  const afterFailure = await page.evaluate(() => JSON.parse(localStorage.getItem('user_focus-preview_journalEntries') || '[]'));
  assert.deepEqual(afterFailure, records);
  assert.deepEqual(errors, []);
  console.log(`PASS: Journal quick review → emotion + lesson saved → reminder survives reload → unpin without duplicate. Desktop/mobile screenshots: ${out}`);
